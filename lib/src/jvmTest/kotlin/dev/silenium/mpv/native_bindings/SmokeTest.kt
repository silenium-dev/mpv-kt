...
import dev.silenium.mpv.TestMpvInstance
...
class SmokeTest {
    ...
    fun nativeSmokeTest() {
        ...
        ret = libmpv.mpv_command_async(
            handle, 1u,
            listOf("loadfile", TestMpvInstance.TEST_VIDEO),
        )
        if (ret != Error.SUCCESS) {
    ...
    fun apiSmokeTest(): Unit = runBlocking {
        ...
        mpv.commandAsync("loadfile", TestMpvInstance.TEST_VIDEO).getOrThrow()
    ...
    companion object {
        private val log = LoggerFactory.getLogger(SmokeTest::class.java)
    }
}
